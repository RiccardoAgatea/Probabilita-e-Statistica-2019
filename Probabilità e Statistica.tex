...
\begin{definizione}
	...
	\end{equation*}
\end{definizione}
L'utilizzo di statistiche per ricavare una stima del valore vero è detta stima puntuale; è una metodologia molto ottimistica, perchè al variare del campione può cambiare pesantemente. Per considerare questa variabilità si può usare la sima per intervalli, cioè ricavare degli intervalli entro i quali è più probabile trovare il valor vero, ma noi non la facciamo. In generle, uno stimatore molto buono del valor medio è la media campionaria, perchè è corretto e consistente. Per stimare la varianza $\sigma^2$ non si può usare lo stimatore
\begin{equation*}
	T=\frac{1}{n}\sum_{i=1}^n(X_i-\overline{X})^2
\end{equation*}
perchè è leggermente distorto: risulta
\begin{equation*}
